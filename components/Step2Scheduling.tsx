...
const Step2Scheduling: React.FC<Step2Props> = ({ data, updateData, onNext, onBack }) => {
  ...
  useEffect(() => {
    if (data.flowType === FlowType.Complete) {
      ...
      // Reset agencies if switching to specific, but don't clear if already specific
      if (data.agencies.length === AGENCIES.length) {
         updateData({ agencies: [] });
      }
    }
  ...
  const handleAgencyChange = (agency: string) => {
    ...
    updateData({ agencies: newAgencies, time: '' }); // Reset time when agencies change
  };
    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="space-y-2">
          <div className="flex items-center p-3 border rounded-lg has-[:checked]:bg-blue-50 has-[:checked]:border-blue-400">
            <input type="radio" id="flowComplete" name="flowType" value={FlowType.Complete} checked={data.flowType === FlowType.Complete} onChange={e => updateData({ flowType: e.target.value as FlowType, time: '' })} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500" />
            <label htmlFor="flowComplete" className="ml-3 block text-sm font-medium text-gray-700">{FlowType.Complete}</label>
          </div>
          <div className="flex items-center p-3 border rounded-lg has-[:checked]:bg-blue-50 has-[:checked]:border-blue-400">
            <input type="radio" id="flowSpecific" name="flowType" value={FlowType.Specific} checked={data.flowType === FlowType.Specific} onChange={e => updateData({ flowType: e.target.value as FlowType })} className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {AGENCIES.map(agency => (
              <div key={agency} className="flex items-center">
                <input id={agency} type="checkbox" checked={data.agencies.includes(agency)} onChange={() => handleAgencyChange(agency)} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
      ...
      <div>
         ...
         <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-2">
            {TIME_SLOTS.map((time, index) => {
                 // The number of required slots is the number of agencies selected.
                 const requiredSlots = data.agencies.length || 1;
                 // A slot is disabled if scheduling from here would exceed available times.
      ...
      <div className="flex justify-between mt-8">
        ...
        <button onClick={handleNextClick} className="bg-blue-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-blue-700 transition-colors">
          Próximo
        </button>
      </div>
    </div>
